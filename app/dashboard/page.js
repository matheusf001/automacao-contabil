...
const TABS = ['empresas', 'extrato', 'regras', 'contas', 'importacao'];
const TAB_LABELS = {
...
export default function Dashboard() {
  ...
  async function salvarContaBancoFixa(codigo) {
    const emp = empresas.find(e => e.id === currentEmpresaId);
    if (!emp) return;
    const { error } = await supabase.from('empresas').update({ conta_banco_fixa: codigo || null }).eq('id', emp.id);
  ...
  async function importarPlano() {
    ...
    for (let i = 0; i < novoPlano.length; i += chunkSize) {
      const chunk = novoPlano.slice(i, i + chunkSize).map(c => ({ ...c, empresa_id: destEmpresaImport }));
  ...
  async function addRegra() {
    const { error } = await supabase.from('regras').insert({
      empresa_id: currentEmpresaId, palavra_chave: '', codigo: 0, descricao: '', ordem: regras.length,
    });
    if (error) { alert('Erro: ' + error.message); return; }
  ...
  async function updateRegra(regra, field, value) {
    const patch = { [field]: field === 'codigo' ? (parseInt(value) || 0) : value };
  ...
  // ---------- PLANO DE CONTAS (edição manual) ----------
  async function addContaManual() {
    const { error } = await supabase.from('plano_contas').insert({ empresa_id: currentEmpresaId, codigo: 0, classificacao: '', descricao: '' });
    if (error) { alert('Erro: ' + error.message); return; }
    ...
    <div className="app">
      ...
      <header className="top">
        <div>
          ...
          <div className="subtitle">{userEmail} · <a href="#" onClick={(e) => { e.preventDefault(); handleLogout(); }} style={{ color: 'var(--teal)' }}>sair</a></div>
        </div>
      ...
      {tab === 'empresas' && (
        <section className="panel">
          ...
          <p className="hint">Cada empresa tem seu próprio plano de contas e regras. Dados no banco — visíveis para todos que fizerem login.</p>
            ...
            {empresas.map(emp => (
              <div key={emp.id} className={'empresa-row' + (emp.id === currentEmpresaId ? ' active' : '')}>
                <div>
                  ...
                  <div className="meta">conta banco fixa: {emp.conta_banco_fixa ?? '—'}</div>
                </div>
                <div className="row" style={{ margin: 0 }}>
                  <button className="btn secondary" onClick={() => setCurrentEmpresaId(emp.id)}>Usar</button>
                  <button className="btn secondary" onClick={() => renomearEmpresa(emp)}>Renomear</button>
                  <button className="btn danger" onClick={() => excluirEmpresa(emp)}>Excluir</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {tab === 'extrato' && (
        <section className="panel">
          ...
          <textarea value={extratoText} onChange={e => setExtratoText(e.target.value)}
            placeholder={'01/07/2026\t1250,00\tPIX RECEBIDO\tC\tCLIENTE XYZ LTDA'} />
          <div className="row">
            <button className="btn teal" onClick={processarExtrato}>Processar extrato</button>
            <button className="btn secondary" onClick={() => { setExtratoText(''); setProcessedRows([]); }}>Limpar</button>
          </div>
          ...
          {processedRows.length > 0 && (
            <>
              <div className="stats">
                ...
                <div className="stat warn">{processedRows.filter(r => r.status !== 'automatico').length} sem correspondência</div>
              </div>
                ...
                <tbody>
                  {processedRows.map((r, i) => (
                    <tr key={i} className={r.status !== 'automatico' ? 'warn-row' : ''}>
                      ...
                      <td>{r.status === 'automatico' ? <span className="badge ok">✔ automatico</span> : <span className="badge warn">⚠ {r.status}</span>}</td>
                    </tr>
                  ))}
                </tbody>
              </table></div>
      ...
      {tab === 'regras' && (
        <section className="panel">
          ...
          <p className="hint">Palavra-chave → código contábil. Quando mais de uma regra combina, prevalece a <strong>última da lista</strong>.</p>
          <div className="row" style={{ marginTop: 0 }}>
            <label style={{ fontSize: 12.5, color: 'var(--ink-soft)' }}>Conta banco (contrapartida fixa):</label>
            <input type="text" list="contas-datalist" placeholder="buscar conta…" style={{ minWidth: 280 }}
              defaultValue={empresaAtiva?.conta_banco_fixa ? `${empresaAtiva.conta_banco_fixa} — ${findContaDesc(empresaAtiva.conta_banco_fixa)}` : ''}
              onBlur={e => salvarContaBancoFixa(extractCodigoFromPicked(e.target.value))} />
            <span className={'save-flag' + (saveFlag ? ' show' : '')}>{saveFlag}</span>
          </div>
          <div className="row">
          ...
          <div className="table-wrap" style={{ marginTop: 14 }}>
            <table>
              <thead><tr><th style={{ width: '34%' }}>PALAVRA-CHAVE</th><th style={{ width: '12%' }}>CÓDIGO</th><th style={{ width: '26%' }}>DESCRIÇÃO CONTA</th><th>OBSERVAÇÃO</th><th style={{ width: 34 }}></th></tr></thead>
              <tbody>
                {regras.map(r => (
                  <tr key={r.id}>
                    <td><input className="cell-edit" defaultValue={r.palavra_chave} onBlur={e => updateRegra(r, 'palavra_chave', e.target.value)} /></td>
                    <td><input className="cell-edit" list="contas-datalist" defaultValue={r.codigo ? `${r.codigo} — ${findContaDesc(r.codigo)}` : ''}
                      onBlur={e => updateRegra(r, 'codigo', extractCodigoFromPicked(e.target.value))} /></td>
                    ...
                    <td><input className="cell-edit" defaultValue={r.descricao || ''} onBlur={e => updateRegra(r, 'descricao', e.target.value)} /></td>
                    <td><button className="del-btn" onClick={() => deleteRegra(r)}>✕</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {tab === 'contas' && (
        <section className="panel">
          ...
          <div className="row" style={{ marginTop: 0 }}>
            ...
            <button className="btn" onClick={addContaManual}>+ Nova conta manual</button>
          </div>
          <div className="table-wrap" style={{ marginTop: 14 }}>
            <table>
              ...
              <tbody>
                {contasFiltradas.map(c => (
                  <tr key={c.id}>
                    <td><input className="cell-edit" defaultValue={c.codigo} onBlur={e => updateConta(c, 'codigo', e.target.value)} /></td>
                    <td><input className="cell-edit" defaultValue={c.classificacao || ''} onBlur={e => updateConta(c, 'classificacao', e.target.value)} /></td>
                    <td><input className="cell-edit" defaultValue={c.descricao} onBlur={e => updateConta(c, 'descricao', e.target.value)} /></td>
                    <td><button className="del-btn" onClick={() => deleteConta(c)}>✕</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
                  ...
                  {processedRows.map((r, i) => {
                    ...
                    return (
                      <tr key={i} className={r.status !== 'automatico' ? 'warn-row' : ''}>
                        ...
                        <td>{r.status === 'automatico' ? <span className="badge ok">✔</span> : <span className="badge warn">⚠</span>}</td>
                      </tr>
                    );
                  })}
                </tbody>
