...
export function contaParaEvento(config, evento) {
  ...
  // Adiantamento NÃO cai na conta de salário quando não configurado: lançar
  // adiantamento como salário estaria contabilmente errado — melhor deixar
  // sem classificar e o usuário decidir.
  if (evento === 'adiantamento') return config.conta_adiantamento || null;
...
export const NOME_EVENTO = { salario: 'salário', ferias: 'férias', rescisao: 'rescisão', decimo: '13º salário', adiantamento: 'adiantamento de salário' };
...
export function cruzarComFolha(row, dadosFolha) {
  ...
  // 3) só o nome (valor diferente do líquido): tratamos como ADIANTAMENTO —
  //    é o padrão dos pagamentos de ~dia 15 (salário integral casa no caso 1).
  //    Só classifica sozinho se a conta de adiantamento estiver configurada.
  const funcs = (dadosFolha.funcionarios || []).filter(f => nomeApareceNoTexto(tokensTexto, f.nome));
  if (funcs.length === 1) return { tipo: 'nome', funcionario: funcs[0], evento: 'adiantamento' };
...
export function descreverRefFolha(ref) {
  ...
  if (ref.tipo === 'nome') {
    ...
    return `funcionário da empresa: ${ref.funcionario.nome}${cargo} — valor diferente do líquido (adiantamento/parcial)`;
  }
  return '';
}
