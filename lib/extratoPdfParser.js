...
export function detectarBanco(texto) {
  if (/Dt\. balancete/i.test(texto) && /Extrato de conta corrente/i.test(texto)) return 'bb';
...
export const NOME_BANCO = { bb: 'Banco do Brasil', sicredi: 'Sicredi', santander: 'Santander', itau: 'Itaú', bradesco: 'Bradesco' };

// ---------- BANCO DO BRASIL ----------
// Linha do lançamento: data ag(4) lote(5) cod(3) histórico documento valor C/D
// (no fim do dia pode vir também o saldo: "... 8.613,74 D 0,00 C").
// O favorecido/remetente vem na LINHA SEGUINTE ("01/06 08:51 FULANO...").
function parseBB(linhas) {
  const RE_BB = /^(\d{2}\/\d{2}\/\d{4})\s+(\d{4})\s+(\d{5})\s+(\d{3})\s+(.+?)\s+(\d{1,3}(?:\.\d{3})*,\d{2})\s+([CD])(?:\s+\d{1,3}(?:\.\d{3})*,\d{2}\s+[CD])?$/;
  const ehBarreira = (l) =>
    /^G\d{10,}/.test(l) ||                                  // protocolo da consulta
    /^\d{2}\/\d{2}\/\d{4} \d{2}:\d{2}:\d{2}$/.test(l) ||    // data/hora da emissão
    /^Consultas - Extrato/i.test(l) ||
    /^Cliente - Conta/i.test(l) ||
    /^Ag[êe]ncia\b/i.test(l) ||
    /^Conta corrente\b/i.test(l) ||
    /^Per[íi]odo do extrato/i.test(l) ||
    /^Lan[çc]amentos$/i.test(l) ||
    /^Dt\. balancete/i.test(l) ||
    /^-{5,}/.test(l) ||
    /^Transa[çc][ãa]o efetuada/i.test(l) ||
    /Atendimento ao Consumidor/i.test(l) ||
    /deficientes auditivos/i.test(l);

  const out = [];
  for (const l of linhas) {
    if (ehBarreira(l)) continue;
    const m = l.match(RE_BB);
    if (!m) {
      // linha sem o formato de lançamento = detalhe do lançamento anterior
      const ant = out[out.length - 1];
      if (ant && l.length <= 80) ant.detalhamento = limpar(ant.detalhamento + ' ' + l);
      continue;
    }
    const [, data, , , , textoMeio, valorStr, letraCD] = m;
    if (/Saldo Anterior/i.test(textoMeio) || /S A L D O/.test(textoMeio)) continue;
    // o documento é o último token só de dígitos e pontos (ex.: 552.799.000.132.456)
    let historico = limpar(textoMeio);
    let detalhe = '';
    const tokens = historico.split(' ');
    const ultimo = tokens[tokens.length - 1];
    if (tokens.length > 1 && /^[\d.]+$/.test(ultimo)) {
      detalhe = 'doc ' + ultimo;
      tokens.pop();
      historico = tokens.join(' ');
    }
    const valor = moneyParaNumero(valorStr);
    out.push({
      data,
      valorNum: letraCD === 'D' ? -valor : valor,
      historico,
...
export function parseExtratoPdfTexto(texto) {
  ...
  if (!banco) {
    return {
      erro: 'Não reconheci o layout deste PDF. Por enquanto leio extratos do Banco do Brasil, Sicredi, Santander, Itaú e Bradesco — para outros bancos, converta em OFX (ex.: ofxfacil.com.br) e envie o .ofx.',
    };
  }
  const linhas = texto.split('\n').map(l => l.trim()).filter(Boolean);
  const parsers = { bb: parseBB, sicredi: parseSicredi, santander: parseSantander, itau: parseItau, bradesco: parseBradesco };
